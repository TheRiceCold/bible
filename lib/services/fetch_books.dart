...
// Class resposible for fetching books based on the provided verses
...
class FetchBooks {
  ...
  static Future<void> execute({ required MainProvider mainProvider }) async {
    ...
    // Iterate through unique book title to organize chapters and verses
    for (var bookTitle in bookTitles) {
      // Filter verses based on the current book title
      List<Verse> availableVerses = verses.where((v) => v.book == bookTitle).toList();

      // Extract unique chapter numbers from the filtered verses
      List<int> availableChapters = availableVerses.map((e) => e.chapter).toSet().toList();
      ...
      // Create a Book object for the current title and its organized chapters
